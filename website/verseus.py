...
class Verse_Test:
    ...
    def __init__(self, current_user=None, note_id=None, completions=0):
        ...
        print("current_user: ", current_user, "note_id: ", note_id)
    ...
    def score_word(word, scores_matrix):
        return sum([scores_matrix[letter] if letter in scores_matrix else 0 for letter in word.upper()])

    def gen_new_blanks(self):
        # Generate new blanks based on completions
        # Get indices where word_blank_positions is 0
        print(self.word_blank_positions)
        new_blank_spots = []
